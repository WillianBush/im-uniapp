...
import { getAppVersion,getImgDomain,getListWithMid,getFooterHotItem } from "../../../common/api";
...
export default {
  ...
  initNetBind({ state, dispatch, commit }) {
			...
			if (resolve) {
			  ...
			  setTimeout(() => {
				...
				if (user) {
				  ...
				  dispatch("socket/WEBSOCKET_INIT",null,{root:true})
			  ...
			  commit(
				...
				setInterval(function () {
				  ...
				  // _this.$store.dispatch("checkHttpHealth");
				}, 10000)
			  );
			}
		  })
  ...
  appHideOrShow({ dispatch, commit }, payload) {
		// app处于后台时
  ...
  clearDataAction({commit}){
	  commit("clearData");
	  commit("chat/clearData",null,{root:true});
	  commit("user/clearData",null,{root:true});
	  commit("socket/clearData",null,{root:true});
  }
};
