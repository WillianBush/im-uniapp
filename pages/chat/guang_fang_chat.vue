<template>
	<view>
		...
		color:#222;height:80upx;">
							...
							<video direction="0" v-else-if="item.bean.psr=='video'" :src="imgUrl+item.bean.txt"></video>
		...
		<view v-show="showItem==1" class="cu-bar foot "
			...
			<view
				...
				<view v-for="(item,index) in 7" :key="item" @tap="showEmotion(index)"
					...
					<image lazy-load :src="getMotionImg(index)" style="width:50upx;height:50upx;margin-top: 10upx;;">
					</image>
				</view>
			</view>
		</view>
