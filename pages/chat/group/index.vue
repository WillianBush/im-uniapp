<template>
	<view ref="topVew" v-if="$store.state.cur_chat_entity"   :style="chatCfg.chatBackgroundImage&&chatCfg.chatBackgroundImage!=''?'background-image: url('+$store.state.img_url+chatCfg.chatBackgroundImage+')':''" style="background-size: 100%;min-height: 100vh;" >
		<group-custom v-if="isGroupChat"backUrl="/pages/index/index" bgColor="bg-blue"  :isBack="true" :nameToLeft="false">
			<block slot="backText">
			...
			<block slot="content" style="margin-left: 66px;">{{$store.state.cur_chat_entity.name}}({{$store.state.cur_chat_entity.memberCount}}人)
				<uni-text  @tap="goMgr(entity.id)" style="font-size: 22px;color: #000;margin-left: 1%;cursor: pointer;" class="lg text-gray cuIcon-more"></uni-text>
			</block></group-custom>
		...
		<scroll-view @scroll="scrollFn" :scroll-into-view="viewId" :scroll-top="scrollTop" scroll-y="true"    ref="chatVew" @tap="clickChat()"  class="cu-chat" style="background: #eee;" :style="'height: calc(100vh - '+CustomBar+'px - 118px - '+(120+InputBottom)+'upx)'" >
		...
		<view v-if="isGroupChat"  class="cu-bar foot input" :style="[{bottom:InputBottom+'upx'}]" style="flex-direction: row;height:180px; width: calc(80% - 54px);left: calc(20% + 54px);background-color: #fff;">
			<!-- #ifndef H5 -->
			...
			<input
					id="testInputg"
					placeholder="请输入信息"
					style="height:160px !important;line-height:30px;width: 100%;"
					confirm-type="send"
					@confirm="send"
			...
			<button  style="top:55px;min-width: 50px;padding:0px!important" v-show="!showjia" @tap.stop="send()"  class="cu-btn bg-green shadow">发送</button>
		...
		<view v-if="!isGroupChat"  class="cu-bar foot input" :style="[{bottom:InputBottom+'upx'}]" style="flex-direction: row;height:180px; width: calc(80% - 54px);left: calc(20% + 54px);background-color: #fff;">
			...
			<input
					...
					class="solid-bottom" :adjust-position="true" :focus="input_is_focus" maxlength="-1" cursor-spacing="10"
			></input>
			<view @tap="ChooseImage2()" style="cursor: pointer;position: absolute;top: 5px; left: 10px;"><text style="font-size: 60upx;color:#3F92F8" class="iconfont icon-zhaopian-cuxiantiao-fill"></text></view>
			<view @tap="ChooseVideo2()" style="cursor: pointer;position: absolute;top: 5px; left: 50px;"><text style="font-size: 60upx;color:#F39F90" class="iconfont icon-paishe"></text></view>
			<view v-if="false" @tap="sendCard2()" style="cursor: pointer;position: absolute;top: 5px; left: 80px;"><text style="font-size: 60upx;color:#FA9B4E" class="iconfont icon-mingpian2"></text></view>
			<view style="cursor: pointer;position: absolute;top: 5px; left: 80px;" class="action" @tap="showItemIndex(1)">
				<text  class="cuIcon-emojifill text-grey" style="position: absolute;top: 0;left: 0px"></text>
			</view>
			<button  style="top:55px;min-width: 50px;padding:0px!important" v-show="!showjia" @tap.stop="send2()"  class="cu-btn bg-green shadow">发送</button>
