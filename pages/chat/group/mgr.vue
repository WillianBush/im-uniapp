			...
			<view>
				<view class="cu-chat">
					<div class="left-icon" @click="logShow = false">
						<image style="width: 10px; height: 16px; float: left; margin-top: 3px"
							src="@/static/images/back.png">
						</image>
						<span style="margin-left: 10px; color: black; font-size: 16px">返回</span>
					</div>
					<view v-if="!chatLogs.length" style="
              text-align: center;
              color: #aaa;
              margin-top: 60upx;
              font-size: 28upx;
            ">
					...
					<div v-else v-for="(item, index) in chatLogs">
						...
						<div v-else-if="item.type == 'USER_CARD'">
							<view v-if="item.bean.fromUid == user.id" class="cu-item self">
								<view class="main">
									<view @tap="clickCard(item.bean)" style="
                    ...
                    ">
                      ...
                      ">
                          ...
                          width: 80upx;
                          height: 80upx;
                        " class="cu-avatar radius" :style="
                          'background-image:url(' +
                          getHeadPic(item.bean.mheadpic) +
                          ');'
                        ">
											</view>
											<view style="
                    ...
                    'background-image:url(' +
                    getHeadPic(item.bean.fromHeadpic) +
                    ');'
                  ">
								</view>
							...
							<view v-else class="cu-item">
								<view @tap.stop="goUserDetail(item.bean.fromUid)" class="cu-avatar radius" :style="
                    'background-image:url(' +
                    getHeadPic(item.bean.fromHeadpic, imgUrl) +
                    ');'
                  ">
								</view>
								<view class="main">
                    ...
                    ">
                      ...
                      ">个人名片</view>
                        ...
                        ">{{ item.bean.subTxt }}"</text>
										...
										<!-- <video
                      direction="0"
                      v-else-if="item.bean.psr == 'video'"
                      :src="imgUrl + item.bean.txt"
                    ></video> -->
		...
		<view style="background: #fff; width: 80%; margin: 10px 0 0 12%" v-if="!logShow">
          ...
          " class="margin-top">
					<view v-if="isEmployee" style="height: 60upx; width: 90%; margin: auto auto">
						<text style="float: left; font-weight: 800; color: #333">群成员</text>
              ...
              ">
							<view @tap="goUserDetail(index)" v-for="(item, index) in curChatEntity.top5Hp"
								class="cu-avatar round margin-left" :style="
                  'height:100upx;width:100upx;background-image:url(' +
                  getHeadPic(item) +
                  ');'
                ">
							</view>
							<text v-if="false" @tap="yaoqi()" style="
                  position: relative;
                  top: 36upx;
                  font-size: 120upx;
                  margin-top: 20upx;
          ...
          " class="cu-list menu">
            ...
            " @tap="lookNotShimingMemberList()" class="cu-item arrow">
              ...
              user.id == curChatEntity.owner_UUID ||
              curChatEntity.memberMgr_ids.indexOf(user.id) >= 0
            " @tap="goAddMember()" class="cu-item arrow">
						<view class="content">
            ...
            " @tap="goSssList" class="cu-item arrow">
              ...
              user.id == curChatEntity.owner_UUID ||
              curChatEntity.memberMgr_ids.indexOf(user.id) >= 0
            " @tap="editGroupName()" class="cu-item arrow margin-top">
						<view class="content">
              ...
              user.id == curChatEntity.owner_UUID ||
              curChatEntity.memberMgr_ids.indexOf(user.id) >= 0
            " @tap="edit_pic()" class="cu-item arrow">
						<view class="content">
							<text class="text-grey" style="color: #333">群组头像</text>
							<view class="cu-avatar round lg" :style="
                  'width:60upx;height:60upx;float: right;background-image: url(' +
                  getHeadPic(curChatEntity.img) +
                  ');'
                ">
							</view>
						</view>
			...
			<GroupDeleteMember v-show="PageCur == 'delete'" :keyid="randomKeyId" @goBack="showGroup">
			</GroupDeleteMember>
...
<script>
	...
	export default {
		...
		computed: {
			...
			...mapState("user", ["user", "isEmployee", "shimingCfg"]),
			...mapState("app", ["imgUrl"]),
		},
		methods: {
			...mapActions("user", ["getShimingCfgAction"]),
			...mapActions("chat", [
				"getRoomConfigAction",
				"transferGroupAction",
				"dissolveGroupAction",
			...
			...mapActions("socket", ["WEBSOCKET_SEND"]),
			getHeadPic(img) {
						...
						if (res_data.code == 201) {
							...
							if (res_data.body && res_data.body.list.length != 0) {
								...
								uni.setStorageSync(
									...
									JSON.stringify(_this.curChatMsgList)
								);
							}
							for (let i = 0; i < res_data.body.list.length; i++) {
								//从[0]中取出
							...
							this.totalCount =
								res_data.body.totalCount * res_data.body.pageCount;
						}
					})
					.catch((error) => {
						uni.showToast({
			...
			getLogs() {
				...
				let str = uni.getStorageSync(
					user.id + "#" + this.curChatEntity.id + "_CHAT_MESSAGE"
				);
				if (str && str != "") {
					...
					success: function(res) {
						if (res.confirm) {
							...
							uni.removeStorageSync(
								user.id + "#" + _this.curChatEntity.id + "_CHAT_MESSAGE"
							);
							_this.setCurChatMsgList([]);
							uni.removeStorageSync(
								user.id +
								"#" +
								_this.curChatEntity.id +
								"_CHAT_MESSAGE_LASTCONTENT"
							);
							uni.removeStorageSync(
								user.id + "#" + _this.curChatEntity.id + "_CHAT_MESSAGE_UNREAD"
							);
							uni.showToast({
								title: "清除成功",
								duration: 2000,
