	...
	<view>
		<view v-show="PageCur=='main'">
			...
			<scroll-view scroll-y class="indexes" :style="[{height:'calc(100vh - 100upx)'}]"
				...
				<view v-if="list.length>0" style="margin-top:10px;padding:20upx 0;background: #fff;"
					class="cu-list menu" :class="[true?'sm-border':'',false?'card-menu ':'']">
					<view v-for="item in list" class="cu-item">
						<view v-if="item.from_member_uuid==user.id" class="content">
							<view class="cu-avatar round lg"
								:style="{'backgroundImage': 'url('+getHeadPic(to_headpic) +')' }"
						...
						<view v-if="item.to_member_uuid==user.id" class="content">
							...
							<button v-if="item.status=='0'" @tap="verify(item.id,2)"
								style="float:right;margin-top:8upx;" class="cu-btn">拒绝</button>
